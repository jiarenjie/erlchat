...
refresh_token(Appid,Reftesh_token) when is_list(Appid),is_list(Reftesh_token) ->
  %Access_token_Rui = "https://api.weixin.qq.com/sns/oauth2/access_token?appid="++Appid++"&secret="++Secret++"&code="++binary_to_list(Code)++"&grant_type=authorization_code",
  REFRESH_TOKEN_Rui = ?API_URL_REFRESH_TOKEN++"?appid="++Appid++"&grant_type=refresh_token&refresh_token="++Reftesh_token,
...
get_userinfo(Access_token,Openid) when is_list(Access_token),is_list(Openid)->
  ...
  Response_Json.
verify(Signature, Timestamp, Nonce,Token) when is_binary(Signature),is_binary(Timestamp),is_binary(Nonce),is_binary(Token) ->
